...
----
{
  ...
  "version": "0.1.0",
  "dependencies": {
    "cloudstate": "^{cloudstate_node_support_version}"
  },
  "scripts": {
...
You can place protobuf files in your project wherever you like, for example, in the root directory, or in a directory named `protos`. In the `package.json` above we've placed the shopping cart application example shown earlier in xref:docs:concepts:grpc.adoc[gRPC descriptors] in a file in the root folder called `shoppingcart.proto`.

== Creating and starting a server

There are two ways to create and start a Cloudstate gRPC server. The first is to create an link:api/module-cloudstate.Entity.html[`Entity`], and invoke link:api/module-cloudstate.Entity.html#start[`start`] on it. This allows creating a server that serves a single entity, with the default options. We'll look at this more in the subsequent pages. Alternatively, you can use the link:api/module-cloudstate.CloudState.html[`CloudState`] class, add one or more entities to it, and then invoke link:api/module-cloudstate.CloudState.html#start[`start`], like so:
...
For the full range of options available on the `CloudState` class, see link:++api/module-cloudstate.CloudState.html#~options++[`CloudState~options`].
