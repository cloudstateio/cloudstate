...
Event sourced entities persist events and snapshots, and these need to be serialized when persisted. A snapshot is the current state of the entity, and so the entities state must be serializable. The most straight forward way to persist events and the state is to use protobufs. Cloudstate will automatically detect if an emitted event or snapshot is a protobuf, and serialize it as such using `protobufjs`. See [https://www.npmjs.com/package/protobufjs](https://www.npmjs.com/package/protobufjs) for more information on `protobufjs`. For other serialization options, including JSON, see xref:serialization.adoc[Serialization].
...
An event sourced entity can be created using the link:api/module-cloudstate.EventSourced.html[EventSourced] class.
...
The `EventSourced` class provides a helper method called link:api/module-cloudstate.EventSourced.html#lookupType[`lookupType`] to facilitate this. So before implementing anything, we'll look up these types so we can use them later.
...
To create the initial state, we set the link:api/module-cloudstate.EventSourced.html#initial[`initial`] callback. This takes the id of the entity being created, and returns a new empty state, in this case, an empty shopping cart:
...
A link:api/module-cloudstate.EventSourced.html#~commandHandler[command handler] is a function that takes a command, the current state, and an link:api/module-cloudstate.EventSourced.EventSourcedCommandContext.html[`EventSourcedCommandContext`]. It implements a service call on the entities gRPC interface.
...
A command handler may emit an event by using the link:api/module-cloudstate.EventSourced.EventSourcedCommandContext.html#emit[`emit`] method on the `EventSourcedCommandContext`.
...
This command handler also validates the command, ensuring the quantity items added is greater than zero. Invoking link:api/module-cloudstate.EventSourced.EventSourcedCommandContext.html#fail[`fail`] fails the command - this method throws so there's no need to explicitly throw an exception.

=== Event handlers

An link:api/module-cloudstate.EventSourced.html#~eventHandler[event handler] is invoked at two points, when restoring entities from the journal, before any commands are handled, and each time a new event is emitted. An event handlers responsibility is to update the state of the entity according to the event. Event handlers are the only place where its safe to mutate the state of the entity at all.
...
Once you have your command handler and event handler functions implemented, you can now set your behavior. The link:api/module-cloudstate.EventSourced.html#~behaviorCallback[behavior callback] takes the current state of the entity, and returns an object with two properties, `commandHandlers` and `eventHandlers`. The callback may return different sets of handlers according to the current state, this will be explored more <<Multiple behaviors,a little later>>, for now we'll just implement an entity with one set of handlers.

The behavior callback can be set by setting the link:api/module-cloudstate.EventSourced.html#behavior[`behavior`] property on the entity:
...
If you only have a single entity, as a convenience, you can start it directly, by invoking the link:api/module-cloudstate.EventSourced.html#start[`start`] method, like so:
