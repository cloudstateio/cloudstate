...
This page documents how to implement Cloudstate event sourced entities in JavaScript. For information on what Cloudstate event sourced entities are, please read the general xref:concepts:eventsource.adoc[Event sourcing] documentation first.

== Persistence types and serialization

Event sourced entities persist events and snapshots, and these need to be serialized when persisted. A snapshot is the current state of the entity, and so the entities state must be serializable. The most straight forward way to persist events and the state is to use protobufs. Cloudstate will automatically detect if an emitted event or snapshot is a protobuf, and serialize it as such using `protobufjs`. See https://www.npmjs.com/package/protobufjs[https://www.npmjs.com/package/protobufjs] for more information on `protobufjs`. For other serialization options, including JSON, see xref:concepts:serialization.adoc[Serialization].
...
An event sourced entity can be created using the {jsdoc_url_prefix}EventSourced.html[EventSourced] {jsdoc_url_prefix}EventSourced.html[image:ROOT:new-tab.svg[title="Open in separate tab", width=16], window="jsdoc"]  class.

[source,js]
----
include::samples:example$js-shopping-cart/shoppingcart.js[tag=entity-class]
----
...
The `EventSourced` class provides a helper method called {jsdoc_url_prefix}EventSourced.html#lookupType[lookupType] {jsdoc_url_prefix}EventSourced.html#lookupType[image:ROOT:new-tab.svg[title="Open in separate tab", width=16], window="jsdoc"] to facilitate this. So before implementing anything, we'll look up these types so we can use them later.

ifdef::review[REVIEWERS, the following, eventsourced/shoppingcart.js,  was not included in the samples directory so I commented out for now.]
...
To create the initial state, we set the {jsdoc_url_prefix}EventSourced.html#initial[initial] {jsdoc_url_prefix}EventSourced.html#initial[image:ROOT:new-tab.svg[title="Open in separate tab", width=16], window="jsdoc"]  callback. This takes the id of the entity being created, and returns a new empty state, in this case, an empty shopping cart:

ifdef::review[Reviewers, the following, eventsourced/shoppingcart.js, was not included in the samples dir, so commented out for now]
////
...
A {jsdoc_url_prefix}EventSourced.html#~commandHandler[command handler] {jsdoc_url_prefix}EventSourced.html#~commandHandler[image:ROOT:new-tab.svg[title="Open in separate tab", width=16], window="jsdoc"] is a function that takes a command, the current state, and an {jsdoc_url_prefix}EventSourced.EventSourcedCommandContext.html[EventSourcedCommandContext].  {jsdoc_url_prefix}EventSourced.EventSourcedCommandContext.html[image:ROOT:new-tab.svg[title="Open in separate tab", width=16], window="jsdoc"] It implements a service call on the entities gRPC interface.
...
A command handler may emit an event by using the {jsdoc_url_prefix}EventSourced.EventSourcedCommandContext.html#emit[emit].  {jsdoc_url_prefix}EventSourced.EventSourcedCommandContext.html#emit[image:ROOT:new-tab.svg[title="Open in separate tab", width=16], window="jsdoc"] method on the `EventSourcedCommandContext`.
...
Once you have your command handler and event handler functions implemented, you can now set your behavior. The {jsdoc_url_prefix}EventSourced.html#~behaviorCallback[behaviorCallback] {jsdoc_url_prefix}EventSourced.html#behaviorCallback[image:ROOT:new-tab.svg[title="Open in separate tab", width=16], window="jsdoc"]  takes the current state of the entity, and returns an object with two properties, `commandHandlers` and `eventHandlers`. The callback may return different sets of handlers according to the current state, as described in <<Multiple behaviors>>. For now we'll just implement an entity with one set of handlers.

The behavior callback can be set by setting the {jsdoc_url_prefix}EventSourced.html#behavior[behavior] {jsdoc_url_prefix}EventSourced.html#behavior[image:ROOT:new-tab.svg[title="Open in separate tab", width=16], window="jsdoc"] property on the entity.
...
If you only have a single entity, as a convenience, you can start it directly, by invoking the {jsdoc_url_prefix}EventSourced.html#start[start] {jsdoc_url_prefix}EventSourced.html#start[image:ROOT:new-tab.svg[title="Open in separate tab", width=16], window="jsdoc"] method.
