...
Your main class will be responsible for creating the Cloudstate gRPC server, registering the entities for it to serve, and starting it. To do this, you can use the https://{javadoc_url_prefix}CloudState.html[CloudState] https://{javadoc_url_prefix}CloudState.html[image:ROOT:new-tab.svg[title="Open in separate tab", width=16], window="JavaDoc"] (io.cloudstate.javasupport.CloudState) server builder, for example:

ifdef::review[REVIEWERS: I don't see io.cloudstate.javasupport.Cloudstate referenced in the Main class.]
...
|https://{javadoc_url_prefix}Context.html[Context] https://{javadoc_url_prefix}Context.html[image:ROOT:new-tab.svg[title="Open in separate tab", width=16], window="JavaDoc"] (`io.cloudstate.javasupport.Context`) 
| N/A                      
| The super type of all Cloudstate contexts. Every invoker makes a subtype of this available for injection, and method or constructor may accept that subtype, or any super type of that subtype that is a subtype of `Context`. 

| `java.lang.String` 
| https://{javadoc_url_prefix}EntityId.html[@EntityId] https://{javadoc_url_prefix}EntityId.html[image:ROOT:new-tab.svg[title="Open in separate tab", width=16], window="JavaDoc"] (`io.cloudstate.javasupport.EntityId`) 
