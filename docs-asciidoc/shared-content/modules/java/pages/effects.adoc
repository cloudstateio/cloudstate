This page documents how to use Cloudstate effects and forwarding in Java. For high level information on what Cloudstate effects and forwarding is, please read the general xref:docs:concepts:effects.adoc[forwarding and effects] documentation first.

== Looking up service call references

To forward a command or emit an effect, a reference to the service call that will be invoked needs to be looked up. This can be done using the link:api/io/cloudstate/javasupport/ServiceCallFactory.html[ServiceCallFactory] interface, which is accessible on any context object via the link:api/io/cloudstate/javasupport/Context.html#serviceCallFactory--[`serviceCallFactory()`] method.
...
Please note that, contrary to command forwarding, the result of the effect is ignored by the current command `addItem`. More details in the common section xref:docs:concepts:effects.adoc[Forwarding and effects]
