...
Installing the Cloudstate operator requires cluster admin, so that the necessary Custom Resource Definitions (CRD) can be installed, and the necessary roles and role bindings can be created. Before you install the Cloudstate operator, make sure that you have Kubernetes {minimum_kubernetes_version} or later, and optionally, Istio {minimum_istio_version} or later.
...
....
cloudstate.operator.proxy.image.cassandra = "cloudstateio/cloudstate-proxy-cassandra:{cloudstate_version}"
....
...
[subs="attributes+"]
....
kubectl apply -n cloudstate -f https://github.com/cloudstateio/cloudstate/releases/download/v{cloudstate_version}/cloudstate-{cloudstate_version}.yaml
....
