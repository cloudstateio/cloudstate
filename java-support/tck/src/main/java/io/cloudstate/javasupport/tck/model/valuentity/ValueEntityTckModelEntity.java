...
package io.cloudstate.javasupport.tck.model.valuentity;
...
@ValueEntity(persistenceId = "value-entity-tck-model")
public class ValueEntityTckModelEntity {
  ...
  public ValueEntityTckModelEntity(Context context) {
    serviceTwoCall =
        context
            .serviceCallFactory()
            .lookup("cloudstate.tck.model.valuentity.ValueEntityTwo", "Call", Request.class);
  }

  @CommandHandler
  public Optional<Response> process(Request request, CommandContext<Persisted> context) {
    boolean forwarding = false;
    for (Valueentity.RequestAction action : request.getActionsList()) {
      switch (action.getActionCase()) {
        ...
        case FORWARD:
          ...
          break;
        case EFFECT:
          Valueentity.Effect effect = action.getEffect();
