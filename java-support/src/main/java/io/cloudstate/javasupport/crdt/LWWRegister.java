...
public interface LWWRegister<T> extends Crdt {
     ...
     * @param customClockValue The custom clock value to use if the clock selected is a custom clock. This is ignored
     *                         if the clock is not a custom clock.
     * @return The old value of the register.
     */
    ...
    enum Clock {
         ...
         * may attach a sequence number to each update, that sequence number can be used to guarantee that the register
         * will converge to the last update emitted by that device.
         */
        CUSTOM,
