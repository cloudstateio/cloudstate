 ...
 * <p/>
 * An Observed-Removed Map allows both the addition and removal of objects in a map. A removal can only be done if
 * all of the additions that caused the key to be in the map have been seen by this node. This means that, for example,
 * if node 1 adds key A, and node 2 also adds key A, then node 1's addition is replicated to node 3, and node 3
 * deletes it before node 2's addition is replicated, then the item will still be in the map because node 2's addition
 * had not yet been observed by node 3. However, if both additions had been replicated to node 3, then the key will be
 * removed.
 * <p/>
 * The values of the map are themselves CRDTs, and hence allow concurrent updates that will eventually converge. Values
 * may only be inserted using the {@link ORMap#getOrCreate(Object, Function)} function, using the {@link CrdtFactory}
 * passed in to the creation callback. Invoking {@link ORMap#put(Object, Crdt)} or any other insertion method will throw
 ...
 * <p/>
 * The map may contain different CRDT types as values, however, for a given key, the type must never change. If two
 * different types for the same key are inserted on different nodes, the CRDT will enter an invalid state that can never
 * be merged, and behavior of the CRDT is undefined.
 * <p/>
...
public interface ORMap<K, V extends Crdt> extends Crdt, Map<K, V> {
     ...
     * @param create A callback used to create the value using the given {@link CrdtFactory} if an entry for the key is
