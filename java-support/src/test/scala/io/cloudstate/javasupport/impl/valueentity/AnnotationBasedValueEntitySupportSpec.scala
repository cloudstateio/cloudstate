...
class AnnotationBasedValueEntitySupportSpec extends WordSpec with Matchers {
  ...
  def create(behavior: AnyRef, methods: ResolvedServiceMethod[_, _]*): ValueEntityHandler =
    new AnnotationBasedValueEntitySupport(behavior.getClass,
                                          anySupport,
                                          methods.map(m => m.descriptor.getName -> m).toMap,
                                          Some(_ => behavior)).create(MockContext)

  def create(clazz: Class[_]): ValueEntityHandler =
    new AnnotationBasedValueEntitySupport(clazz, anySupport, Map.empty, None).create(MockContext)
  ...
  "Value entity annotation support" should {
