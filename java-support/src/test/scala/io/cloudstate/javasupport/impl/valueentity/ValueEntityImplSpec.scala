...
import io.cloudstate.javasupport.valueentity.{CommandContext, CommandHandler, ValueEntity}
...
class ValueEntityImplSpec extends WordSpec with Matchers with BeforeAndAfterAll {
  ...
  private val service: TestValueEntityService = ShoppingCart.testService
...
object ValueEntityImplSpec {
  object ShoppingCart {
    ...
    def testService: TestValueEntityService = service[TestCart]

    def service[T: ClassTag]: TestValueEntityService =
      TestValueEntity.service[T](
    ...
    @ValueEntity(persistenceId = "crud-shopping-cart")
