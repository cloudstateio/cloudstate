...
import io.cloudstate.javasupport.crud.{CommandContext, CommandHandler, CrudEntity}
...
class CrudImplSpec extends WordSpec with Matchers with BeforeAndAfterAll {
  import CrudImplSpec._
  import CrudMessages._
  import ShoppingCart.Item
  import ShoppingCart.Protocol._

  val service: TestCrudService = ShoppingCart.testService
  ...
  "CrudImpl" should {
    "fail when first message is not init" in {
      service.expectLogError("Terminating entity due to unexpected failure") {
        val entity = protocol.crud.connect()
        entity.send(command(1, "cart", "command"))
        entity.expect(failure("Protocol error: Expected Init message for CRUD entity"))
        entity.expectClosed()
      }
    }
    ...
    "fail when entity is sent multiple init" in {
      service.expectLogError("Terminating entity [cart] due to unexpected failure") {
        ...
        entity.send(init(ShoppingCart.Name, "cart"))
        entity.expect(failure("Protocol error: CRUD Entity already inited"))
        entity.expectClosed()
      }
    }
    ...
    "fail when command entity id is incorrect" in {
      service.expectLogError("Terminating entity [cart2] due to unexpected failure for command [foo]") {
        ...
        entity.expect(failure(1, "Protocol error: Receiving CRUD entity is not the intended recipient of command"))
        entity.expectClosed()
      }
    }
    ...
    "fail when command payload is missing" in {
      service.expectLogError("Terminating entity [cart] due to unexpected failure for command [foo]") {
        ...
        entity.expect(failure(1, "Protocol error: No command payload for CRUD entity"))
        entity.expectClosed()
      }
    }
    ...
    "fail when entity is sent empty message" in {
      service.expectLogError("Terminating entity [cart] due to unexpected failure") {
        ...
        entity.expect(failure("Protocol error: CRUD entity received empty/unknown message"))
        entity.expectClosed()
      }
    }
    ...
    "fail action when command handler uses context fail" in {
      service.expectLogError(
        "Fail invoked for command [AddItem] for CRUD entity [cart]: Cannot add negative quantity of item [foo]"
      ) {
        val entity = protocol.crud.connect()
        entity.send(init(ShoppingCart.Name, "cart"))
        ...
        entity.passivate()
        val reactivated = protocol.crud.connect()
    ...
    "fail when command handler throws exception" in {
      service.expectLogError("Terminating entity [cart] due to unexpected failure for command [RemoveItem]") {
        ...
        entity.expect(failure(1, "CRUD entity unexpected failure: Boom: foo"))
        entity.expectClosed()
      }
    }
...
object CrudImplSpec {
  object ShoppingCart {
    ...
    def testService: TestCrudService = service[TestCart]

    def service[T: ClassTag]: TestCrudService =
      TestCrud.service[T](
