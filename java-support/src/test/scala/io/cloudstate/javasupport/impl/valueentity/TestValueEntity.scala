...
object TestValueEntity {
  def service[T: ClassTag](descriptor: ServiceDescriptor, fileDescriptors: FileDescriptor*): TestValueEntityService =
    new TestValueEntityService(implicitly[ClassTag[T]].runtimeClass, descriptor, fileDescriptors)
}

class TestValueEntityService(entityClass: Class[_],
