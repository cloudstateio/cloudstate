...
func (r *StatefulStoreReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, statefulStore); err != nil {
		log.Error(err, "unable to fetch StatefulStore")
		return ctrl.Result{}, ignoreNotFound(err)
	}

	conditions, updated, err := r.Stores.ReconcileStatefulStore(ctx, statefulStore)
	if err != nil {
	...
	for idx := range conditions {
		if conditions[idx].Status != corev1.ConditionFalse {
			summary = conditions[idx].Reason
			break
		}
	}
	if statefulStore.Status.Summary != summary {
		updated = true
		statefulStore.Status.Summary = summary
	}

	updatedConditions := reconciliation.ReconcileStatefulServiceConditions(statefulStore.Status.Conditions, conditions)
	if len(updatedConditions) > 0 || updated {
		if len(updatedConditions) > 0 {
			statefulStore.Status.Conditions = updatedConditions
		}
		if err := r.Status().Update(ctx, statefulStore); err != nil {
			return ctrl.Result{}, err
		}
	}
