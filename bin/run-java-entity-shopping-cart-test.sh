#!/usr/bin/env bash
#
# Run the Java Entity shopping cart sample as a test, with a given persistence store.
#
# run-java-entity-shopping-cart-test.sh [inmemory|postgres]

set -e

echo
echo "=== Running java-entity-shopping-cart test ==="
echo
...
# Build the java entity shopping cart
if [ "$build" == true ] ; then
  echo
  echo "Building java-entity-shopping-cart ..."
  [ -f docker-env.sh ] && source docker-env.sh
  sbt -Ddocker.username=cloudstatedev -Ddocker.tag=dev java-valueentity-shopping-cart/docker:publishLocal
fi
...
case "$store" in
  ...
  postgres ) # deploy the entity-shopping-cart with postgres store
...
spec:
  storeConfig:
    statefulStore:
      name: $statefulstore
  containers:
    - image: cloudstateio/java-valueentity-shopping-cart:latest
      imagePullPolicy: Never
      name: user-function
YAML
;;

  inmemory | * ) # deploy the entity-shopping-cart with in-memory store
...
for i in {1..9} ; do
  cart_id="test$i"
  echo
  echo "Testing entity shopping cart $cart_id ..."

  initial_cart=$(curl -s $url/ve/carts/$cart_id)
