...
import io.cloudstate.protocol.value_entity.{
  ValueEntityProtocol,
...
class TestValueEntityService(context: TestServiceContext) {
  ...
  def handler: PartialFunction[HttpRequest, Future[HttpResponse]] =
    ValueEntityProtocolHandler.partial(testCrud)(context.system)
}

object TestValueEntityService {
  def entitySpec(service: ServiceDescription): EntitySpec =
    TestEntityDiscoveryService.entitySpec(ValueEntityProtocol.name, service)

  def entitySpec(descriptors: Seq[ServiceDescriptor]): EntitySpec =
    TestEntityDiscoveryService.entitySpec(ValueEntityProtocol.name, descriptors)

  final class TestCrud(context: TestServiceContext) extends ValueEntityProtocol {
