...
final class InterceptValueEntityService(context: InterceptorContext) {
  ...
  def handler: PartialFunction[HttpRequest, Future[HttpResponse]] =
    ValueEntityProtocolHandler.partial(interceptor)(context.system)
...
object InterceptValueEntityService {
  ...
  final class CrudInterceptor(context: InterceptorContext) extends ValueEntityProtocol {
    private val client = ValueEntityProtocolClient(context.clientSettings)(context.system)
