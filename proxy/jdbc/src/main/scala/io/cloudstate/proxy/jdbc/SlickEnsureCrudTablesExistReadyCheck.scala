...
import io.cloudstate.proxy.crud.store.{JdbcCrudStateTable, JdbcCrudStateTableConfiguration, JdbcSlickDatabase}
...
class SlickEnsureCrudTablesExistReadyCheck(system: ActorSystem) extends (() => Future[Boolean]) {
...
private class EnsureCrudTablesExistsActor(db: JdbcSlickDatabase) extends Actor with ActorLogging {
  ...
  private val stateCfg = new JdbcCrudStateTableConfiguration(
    context.system.settings.config.getConfig("cloudstate.proxy.crud.jdbc-state-store")
  )

  private val stateTable = new JdbcCrudStateTable {
    override val crudStateTableCfg: JdbcCrudStateTableConfiguration = stateCfg
    override val profile: JdbcProfile = EnsureCrudTablesExistsActor.this.profile
  }

  private val stateStatements = stateTable.CrudStateTableQuery.schema.createStatements.toSeq
