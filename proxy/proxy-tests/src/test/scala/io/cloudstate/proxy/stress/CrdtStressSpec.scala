...
import com.example.crdts.{CrdtExampleClient, Get, MutateSet, SomeValue, UpdateCounter}
...
/**
  ...
  */
class CrdtStressSpec extends WordSpec
  ...
  "CRDT stress tests" should {
    ...
    "work for GCounter" in {
      val countTo = sys.props.getOrElse("gcounter-count-to", "1000").toInt
      println(s"GCounter counting to $countTo...")
      Await.result(Source(1 to countTo)
        .mapAsync(parallelism) { idx =>
          getClient(Random.nextInt(numberOfNodes)).incrementGCounter(UpdateCounter("gcounter-count-to", 1))
            .map(_ => idx)
        }
        .via(reportProgress(countTo))
        .runWith(Sink.ignore), 10.minutes)
    ...
    "work for PNCounter" in {
      ...
      Await.result(Source(1 to pnCounterOps)
        .mapAsync(parallelism) { idx =>
          val update = Random.nextInt(10) - 5
          local.getAndAdd(update)
          getClient(Random.nextInt(numberOfNodes)).updatePNCounter(UpdateCounter("pncounter", update))
            .map(_ => idx)
        }
    ...
    "work for GSet" in {
      ...
      Await.result(Source(1 to gsetOps)
        .mapAsync(parallelism) { idx =>
          ...
          getClient(Random.nextInt(numberOfNodes)).mutateGSet(MutateSet(key = "gset", add = Seq(value)))
            .map(_ => idx)
        }
    ...
    "work for ORSet" in {
      ...
      Await.result(Source(0 until orsetOps)
        .mapAsync(parallelism) { idx =>
          ...
          getClient(clientId).mutateORSet(op)
            .map(_ => idx)
        }
...
object CrdtStressSpec {
  def startNode(httpPort: Int, seedNode: Option[ActorSystem]): (ActorRef, ActorSystem) = {
    ...
    val serverConfig = new EntityDiscoveryManager.Configuration(c)
      .copy(httpPort = httpPort, devMode = true)
    ...
    // Start it
    val manager = system.actorOf(BackoffSupervisor.props(
