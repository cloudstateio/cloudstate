...
package io.cloudstate.proxy.cassandra
...
class CassandraProjectionSupport(implicit system: ActorSystem[_]) extends ProjectionSupport {
  override def create[Offset, Envelope](
    ...
    CassandraProjection.atLeastOnceFlow(projectionId, sourceProvider, flow)

  override def prepare(): Future[Done] = CassandraProjection.createOffsetTableIfNotExists()
}
