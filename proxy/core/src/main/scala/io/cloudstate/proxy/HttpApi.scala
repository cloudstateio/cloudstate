...
object HttpApi {
  ...
  final class HttpEndpoint(
    ...
    final val (methodPattern, pathTemplate, pathExtractor, bodyDescriptor, responseBodyDescriptor) =
      extractAndValidate()
  ...
  final def serve(
    ...
    OpenAPIv3.route(endpoints) match {
      case Some(openApiRoute) =>
        (endpoints: List[PartialFunction[HttpRequest, Future[HttpResponse]]])
          .reduce((previous, current) => current orElse previous) orElse openApiRoute
      case None => NoMatch
    }
  }
}
...
// Possible interesting tools:
//   https://github.com/IBM/openapi-validator
//   https://github.com/mermade/openapi-gui
private object OpenAPIv3 {
  final val `text/yaml` = MediaType.textWithFixedCharset("yaml", HttpCharsets.`UTF-8`, "yaml")

  // FIXME
  // - make sure to escape all data entered into the yaml, to make sure that it is well-formed
  // - run an open-api validator on different permutations to make sure that it is well-formed
  def openApiV3YamlSpec(endpoints: List[HttpApi.HttpEndpoint]): Option[String] =
    endpoints match {
      case Nil => None
      case _ =>
        def headers = {
          val version: String = "FIXME" // FIXME can we obtain this from what the user function responds with?
          val title: String = "FIXME" // FIXME we should be able to craft something here based on the metadata
          val license: String = "FIXME" // FIXME this field might be tricky to get right, so we might have to omit it.
          s"""      |# Generated by Cloudstate
          |openapi: "3.0.0"
          |info:
          |  version: $version
          |  title: $title
        ...
        def servers = {
          // FIXME Is there any way to obtain the servers from within, or even just returning the request's destination?
          val serverURLs: List[String] = List("FIXME")
          s"""       |servers:${serverURLs.map(url => s"\n|  - url: $url").mkString}"""
        }

        def paths = {
          // FIXME don't forget to take into account google.api.HttpBody since it will dictate the content type produced
          def pathFor(endpoint: HttpApi.HttpEndpoint): String = {
            val pattern = endpoint.pathTemplate.path
            s"""       |${pattern}:
           |  ${endpoint.methodPattern.name.toLowerCase}:
           |  operationId:${endpoint.methDesc.getFullName}
           |  tags:
           |  parameters:
           |  responses:
           |    '200':
          ...
          s"""       |paths:\n${endpoints.map(pathFor).mkString}"""
        }

        def components = {
          // FIXME make sure to add support for most-if-not-all proto schema like `oneof` etc
          def schemaFor(typeDescriptor: Descriptor): String =
            s"""       |    <TYPENAME FIXME>:
           |      type: <FIXME object, …>
           |      required: <FIXME list of names of required fields>
           |      properties:
           |       <FIXME field name>:
           |         type: <FIXME typename>
           |         format: <FIXME formatname>"""

          val componentDescriptors =
            endpoints
              .foldLeft(Map.empty[String, Descriptor]) { (map, endpoint) =>
                map // FIXME read below
              // FIXME recursively add all component types from getInputType and getOutputType
              // FIXME don't forget to check if we need to traverse each of the input and output types' nested types
              }
              .values
              .toSet
          s"""       |components:
           |  schemas:${componentDescriptors.map(schemaFor).mkString}"""
        }

        // Assembles the complete descriptor
        Some(s"${headers}\n${servers}\n${paths}\n${components}\n".stripMargin)
    }

  def route(endpoints: List[HttpApi.HttpEndpoint]): Option[PartialFunction[HttpRequest, Future[HttpResponse]]] =
    openApiV3YamlSpec(endpoints) map { spec =>
      {
        case HttpRequest(HttpMethods.GET, Uri.Path("/openapi.yaml"), _, _, _) =>
          Future.successful(HttpResponse(200, entity = HttpEntity(`text/yaml`, spec)))
      }
    }
}
