...
import io.cloudstate.proxy.valueentity.store.{JdbcRepositoryImpl, JdbcStoreSupport}
import io.cloudstate.proxy.entity.{EntityCommand, UserFunctionReply}

import scala.concurrent.{ExecutionContext, Future}

class ValueEntitySupportFactory(
    system: ActorSystem,
  ...
  private val valueEntityClient = ValueEntityProtocolClient(grpcClientSettings)(system)
...
private class ValueEntitySupport(crudEntity: ActorRef, parallelism: Int, private implicit val relayTimeout: Timeout)
