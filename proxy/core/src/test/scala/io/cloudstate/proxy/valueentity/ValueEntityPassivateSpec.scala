...
import io.cloudstate.proxy.valueentity.store.{JdbcInMemoryStore, JdbcRepositoryImpl}
import io.cloudstate.testkit.TestService
import io.cloudstate.testkit.valuentity.ValueEntityMessages

import scala.concurrent.duration._

class ValueEntityPassivateSpec extends AbstractTelemetrySpec {
  ...
  "ValueEntity" should {
    ...
    "restart entity after passivation" in withTestKit(
      ...
      val repository = new JdbcRepositoryImpl(new JdbcInMemoryStore)
