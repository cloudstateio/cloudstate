...
class StatelessHandler {
  ...
  setupUnaryOutContext() {
    ...
    this.ctx.forward = (method, message) => {
      this.ensureNotCancelled();
      this.streamDebug("Forwarding to %s", method);
      const forward = this.support.effectSerializer.serializeEffect(method, message);
    ...
    this.ctx.effect = (method, message, synchronous) => {
      this.ensureNotCancelled();
      this.streamDebug("Emitting effect to %s", method);
      effects.push(this.support.effectSerializer.serializeEffect(method, message, synchronous));
    };
  ...
  setupStreamedOutContext() {
    ...
    this.ctx.effect = (method, message, synchronous) => {
      ...
      this.streamDebug("Emitting effect to %s", method);
      this.call.write({
        sideEffects: [this.support.effectSerializer.serializeSideEffect(method, message, synchronous)]
      });
    };
